...
'lodash.merge');var _lodashMerge2 = _interopRequireDefault(_lodashMerge);var _mkdirpThen = require(
'mkdirp-then');var _mkdirpThen2 = _interopRequireDefault(_mkdirpThen);var _getStdin = require(
'get-stdin');var _getStdin2 = _interopRequireDefault(_getStdin);
...
function addHandlebarsHelpers(files) {
  files.forEach(function (file) {
    ...
    var handlebarsHelper = require(file); // eslint-disable-line global-require
...
function getStdinData() {var 
  text;return _regeneratorRuntime.async(function getStdinData$(context$1$0) {while (1) switch (context$1$0.prev = context$1$0.next) {case 0:context$1$0.next = 2;return _regeneratorRuntime.awrap(_getStdin2['default']());case 2:text = context$1$0.sent;context$1$0.prev = 3;

        debug('Attempting to parse ' + text + ' as JSON');return context$1$0.abrupt('return', 
        JSON.parse(text));case 8:context$1$0.prev = 8;context$1$0.t0 = context$1$0['catch'](3);throw (

          new Error('stdin cannot be parsed as JSON'));case 11:case 'end':return context$1$0.stop();}}, null, this, [[3, 8]]);}



function renderHandlebarsTemplate(
files) {var outputDirectory = arguments.length <= 1 || arguments[1] === undefined ? process.cwd() : arguments[1];var 
  outputExtension = arguments.length <= 2 || arguments[2] === undefined ? 'html' : arguments[2];var data = arguments.length <= 3 || arguments[3] === undefined ? {} : arguments[3];var stdout = arguments.length <= 4 || arguments[4] === undefined ? false : arguments[4];return _regeneratorRuntime.async(function renderHandlebarsTemplate$(context$1$0) {while (1) switch (context$1$0.prev = context$1$0.next) {case 0:context$1$0.next = 2;return _regeneratorRuntime.awrap(
        _Promise.all(files.map(function renderTemplate(file) {var 

          path, 
          htmlContents;return _regeneratorRuntime.async(function renderTemplate$(context$2$0) {while (1) switch (context$2$0.prev = context$2$0.next) {case 0:debug('Rendering template ' + file + ' with data', data);path = _path.resolve(outputDirectory, _path.basename(file, _path.extname(file)) + '.' + outputExtension);context$2$0.t0 = _handlebars2['default'];context$2$0.next = 5;return _regeneratorRuntime.awrap(_fsPromise.readFile(file, 'utf8'));case 5:context$2$0.t1 = context$2$0.sent;context$2$0.t2 = data;htmlContents = context$2$0.t0.compile.call(context$2$0.t0, context$2$0.t1)(context$2$0.t2);if (!
                stdout) {context$2$0.next = 13;break;}context$2$0.next = 11;return _regeneratorRuntime.awrap(
                process.stdout.write(htmlContents, 'utf8'));case 11:context$2$0.next = 19;break;case 13:context$2$0.next = 15;return _regeneratorRuntime.awrap(

                _mkdirpThen2['default'](outputDirectory));case 15:context$2$0.next = 17;return _regeneratorRuntime.awrap(
                _fsPromise.writeFile(path, htmlContents, 'utf8'));case 17:
                debug('Wrote ' + path);
                console.error('Wrote ' + path + ' from ' + file);case 19:case 'end':return context$2$0.stop();}}, null, this);})));case 2:case 'end':return context$1$0.stop();}}, null, this);}
...
if (require.main === module) {(function () {
    ...
    {(function () {
        ...
        if (options.stdin) {
          ...
          setup.push(getStdinData().then(function (stdinData) {return data = stdinData;}));}

        _Promise.all(setup).
        then(function () {return expandGlobList(options._);}).
        then(function (files) {return renderHandlebarsTemplate(files, options.output, options.extension, data, options.stdout);})['catch'](
        function (error) {
